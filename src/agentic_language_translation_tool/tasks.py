...
def render_verification_batch(batch: Batch, segments: list[Segment]) -> str:
    ...
    lines = [
        ...
        "",
        "Use the source and target languages recorded in `job.json`; do not assume",
        "English is either side of the translation.",
        "Examples: keep version strings like `v2.1`, percentages like `15%`,",
        "identifiers like `API_KEY`, and quoted UI labels like \"Export\" stable",
        "unless the glossary says otherwise. If Apple refers to the technology",
        "company, do not treat a literal translation such as German `Apfel` as",
        "acceptable.",
        "",
...
def render_correction_batch(
    ...
    lines = [
        ...
        "Use the source and target languages recorded in `job.json`; preserve",
        "version strings, percentages, identifiers, quoted labels, and product or",
        "acronym identifiers unless the glossary says otherwise.",
